...
| 包 | 适用场景 | FFmpeg |
| --- | --- | --- |
| [Full 安装版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.2.3/FFCodec-Lab-Setup-Full-1.2.3.exe) | 安装后直接使用 | 内置 `ffmpeg`、`ffprobe`、`ffplay` |
| [Base 安装版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.2.3/FFCodec-Lab-Setup-Base-1.2.3.exe) | 已有 FFmpeg，或自行管理版本 | 不包含 |
| [Onedir 目录版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.2.3/FFCodec-Lab-Onedir-1.2.3.zip) | 免安装、解压即用 | 不包含 |

下载后可使用 [SHA256SUMS.txt](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.2.3/SHA256SUMS.txt) 校验文件完整性。Base 和 Onedir 会自动搜索系统 PATH、同目录和常见目录中的 FFmpeg，也可以在设置中选择自定义文件夹。Full 包会严格确认候选程序的身份，避免把 `ffprobe.exe` 当作 FFmpeg 而显示 `unknown`。
...
该命令执行全部 TypeScript 类型检查、Web/Desktop Vitest 测试和 ESLint。`v1.2.3` 发布门禁为 Web `590/590`、Desktop `29/29`，并额外通过 Web/Desktop production build、Windows 三种包型构建、安装器内层资源检查和 Onedir ZIP 完整性测试。
