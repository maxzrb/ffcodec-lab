...
| 包 | 适用场景 | FFmpeg |
| --- | --- | --- |
| [Full 安装版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.7.0/FFCodec-Lab-Setup-Full-1.7.0.exe) | 安装后直接使用 | 内置 `ffmpeg`、`ffprobe`、`ffplay` |
| [Base 安装版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.7.0/FFCodec-Lab-Setup-Base-1.7.0.exe) | 已有 FFmpeg，或自行管理版本 | 不包含 |
| [Onedir 目录版](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.7.0/FFCodec-Lab-Onedir-1.7.0.zip) | 免安装、解压即用 | 不包含 |

下载后可使用 [SHA256SUMS.txt](https://github.com/maxzrb/ffcodec-lab/releases/download/v1.7.0/SHA256SUMS.txt) 校验文件完整性。Base 和 Onedir 会自动搜索系统 PATH、同目录和常见目录中的 FFmpeg，也可以在设置中选择自定义文件夹。Full 包会严格确认候选程序的身份，避免把 `ffprobe.exe` 当作 FFmpeg 而显示 `unknown`。
...
`pnpm check` 执行全部 TypeScript 类型检查、Web/Desktop Vitest 测试和 ESLint。`pnpm verify:filter-formats` 优先调用 PATH 中的 FFmpeg，并在 Windows 仓库中交叉检查随附构建，实证验证高精度候选格式、采样/Alpha 保持、字幕、画面调整、降位抖动及已知 8-bit-only 滤镜。`v1.7.0` 发布门禁包括 Web/Desktop 全量测试、目录审计、双端 production build、两套 FFmpeg 高精度格式验证、Windows 三种包型构建、安装器内层资源检查、Onedir ZIP 完整性测试和 SHA-256 校验。
