...
- 自定义标题栏在“解锁窗口大小”左侧增加性能监控按钮，作为固定且唯一的详情页入口。
- 命令检查器下方的性能摘要卡片改为用户可选，默认关闭。
- CPU、GPU 使用率使用两个汽车速度表/转速表风格的大型圆形仪表。
- 内存使用率使用油量表风格的小型仪表。
- 摘要卡片只展示状态，不再通过点击进入详情页。
...
- Windows 首次启动默认仅请求提升 helper；用户拒绝 UAC 或提升启动器不可用时自动回退普通权限。权限不足、传感器无值或设备不支持时返回 capability/diagnostic，不让进程失败。
...
在自定义标题栏的“解锁窗口大小”按钮左侧新增“性能监控”按钮：

- 图标建议使用统一 1px 线条绘制的波形/仪表，不使用 emoji。
- 中文显示“性能监控”，英文显示“Performance”；窗口宽度不足时允许只显示图标。
- 按钮是性能详情页的固定且唯一入口，始终可用，不依赖当前工作台模块或命令 Tab。
- helper 正常时可显示绿色状态点；未启动使用中性灰；权限受限使用黄色；故障使用红色。
- 点击后打开 Desktop renderer 内的全屏性能页；再次点击或页面关闭按钮返回工作台。
- 该入口不放入共享 Web header，Web 不显示。

### 5.2 可选的主页面摘要

通过 Desktop-only `inspectorWidgets` 扩展点将摘要注入命令检查器下方，不把硬件逻辑写入共享 Web 页面。

- 默认关闭，由性能详情页中的“在主界面显示性能摘要”开关控制。
- 开关首版保存为 Desktop local storage 项 `ffcodec-desktop-performance-summary-enabled`；后续随 FFmpeg 路径一起迁入统一 Desktop 持久化存储。
- 关闭时不保留空白占位，也不增加命令检查器高度。
...
- 摘要不是导航入口，不设置整卡点击；仪表只提供可访问状态文本。进入详情页统一使用标题栏按钮。
...
- 仪表使用 SVG；时间序列使用固定版本 `uPlot 1.6.32` 的高 DPI Canvas。CPU 每个逻辑线程独立成图，GPU 与内存使用同风格大图。
- 支持 `prefers-reduced-motion`，指针变化使用短插值但不持续弹簧动画。

## 6. 采样、性能与编码联动

- 默认按需启动：用户点击标题栏“性能监控”按钮时启动 helper。详情页打开时保持 1000ms，首版不追求 60 FPS 假实时。
- 仪表指针可在 renderer 内以 150–250ms 插值，数据采集频率不因此提高。
- UI 不可见或窗口最小化时降为 3000ms；恢复后立即请求一次快照。
- 关闭详情页后，如果主页面摘要已开启则继续采样；摘要关闭时延迟 10 秒停止 helper，避免误触或快速往返造成频繁进程启动。
...
- FFCodec Lab、renderer 与 FFmpeg 始终不整体提权；只读硬件采集 helper 单独请求管理员权限。
- 非管理员拿不到的传感器显示“权限或硬件不支持”，基础 CPU/GPU/内存负载能读多少显示多少。
- 用户首次打开性能监控时由 Windows 显示标准 UAC 对话框；无法绕过系统确认。拒绝或启动失败时自动回退普通权限 helper，不提升 Electron renderer。
...
### Phase PM-2：标题栏入口与可选摘要

- 自定义标题栏在尺寸解锁按钮左侧新增性能监控入口，并完成全屏性能页壳层、返回和语言同步。
- 新增 `inspectorWidgets` 扩展点和默认关闭的 Desktop 性能摘要组件。
- 性能页提供“在主界面显示性能摘要”持久化开关。
- SVG 实现 CPU/GPU 大表、内存小表和缺失态；摘要卡片不承担页面导航。
- 完成右侧检查器高度自适应吸附，专门回归此前页面底部显示问题。

验收：1280×720 锁定窗口下标题栏性能按钮、尺寸解锁按钮及窗口控制保持同一行；摘要关闭时命令检查器布局与当前版本一致；摘要开启时不截断；Web bundle 不包含监控组件。
...
- React：标题栏入口、摘要偏好、仪表值、不可用态、设备切换、详情页开关、listener 清理、键盘操作。
...
- helper 可单独提权、只读、无网络；拒绝提权时回退普通权限；性能采样与历史不持久化，仅保存“显示主页面摘要”的界面偏好。
- 摘要 SVG + 详情 uPlot Canvas；固定 `uPlot 1.6.32` 并随包携带 MIT license。
- 标题栏按钮是性能页唯一入口；主页面摘要默认关闭且不可点击跳转。
- 点击标题栏入口时按需启动 helper；关闭详情且摘要关闭时延迟停止。
