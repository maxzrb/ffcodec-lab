...
export function resolveInputSection(
  ...
  const fields: ResolvedField[] = [
    ...
    // -- 保留全部视频流开关 ------------------------------------------------
    {
      ...
      disabledReason: !videoActive ? '当前不输出视频（-vn），流选择不可用。' : undefined,
    },
    {
      id: 'streams.videoStreams', label: '视频流', controlType: 'multiselect',
      description: !videoActive ? '当前不输出视频（-vn）。' : videoEncMode ? '勾选的视频流参与输出。默认编码，可在右侧逐项切换为复制。' : '视频流将原样复制（-c:v copy）。',
      value: videoIndices,
      options: streamIndexOptions,
      visible: !preserveAllVideo, disabled: !videoActive,
      disabledReason: !videoActive ? '当前不输出视频（-vn）。' : undefined,
    ...
    // -- 保留全部音频流开关 ------------------------------------------------
    {
      ...
      disabledReason: !audioActive ? '当前不输出音频（-an），流选择不可用。' : undefined,
    },
    {
      id: 'streams.audioStreams', label: '音频流', controlType: 'multiselect',
      description: !audioActive ? '当前不输出音频（-an）。' : audioEncMode ? '勾选的音频流参与输出。默认编码，可在右侧逐项切换为复制。' : '音频流将原样复制（-c:a copy）。',
      value: audioIndices,
      options: streamIndexOptions,
      visible: !preserveAllAudio, disabled: !audioActive,
      disabledReason: !audioActive ? '当前不输出音频（-an）。' : undefined,
      sourceRefs: [], verificationLevel: 'project-derived', needsCrossVerification: false,
    ...
    ),
    {
      ...
      description: preserveAllSubtitle ? '当前保留全部字幕流。关闭后可手动选择或留空不输出字幕。' : '勾选的字幕流参与输出，留空则不输出字幕。',
