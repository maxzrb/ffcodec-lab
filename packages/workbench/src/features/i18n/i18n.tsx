...
const ENGLISH_TEXT: Record<string, string> = {
  ...
  '只作用于主输入。没有明确需求时保持全部“不设置”；硬件加速可用性必须用短样片实测。': 'Applies only to the primary input. Leave everything unset unless needed, and verify hardware acceleration with a short sample.',
  '硬件加速解码方式 (-hwaccel)': 'Hardware-accelerated decoding (-hwaccel)',
  '不设置（使用 FFmpeg 默认解码）': 'Unset (use FFmpeg default decoding)',
  '不设置时由 FFmpeg 使用普通软件解码。所选方式仍需当前 FFmpeg 构建、操作系统、GPU、驱动和输入编码格式共同支持。': 'When unset, FFmpeg uses normal software decoding. The selected method still requires support from the FFmpeg build, OS, GPU, driver, and source codec.',
  'Windows Direct3D 11 视频解码。': 'Windows Direct3D 11 video decoding.',
  'Windows Direct3D 12 视频解码，需要较新的 FFmpeg、驱动与硬件。': 'Windows Direct3D 12 video decoding; requires a recent FFmpeg build, driver, and hardware.',
  'NVIDIA CUDA/NVDEC 路径。': 'NVIDIA CUDA/NVDEC path.',
  'Intel Quick Sync Video。': 'Intel Quick Sync Video.',
  'AMD Advanced Media Framework。': 'AMD Advanced Media Framework.',
  'Vulkan 硬件设备路径，实际解码支持依赖构建与驱动。': 'Vulkan hardware-device path; actual decoding support depends on the build and driver.',
  '较旧的 Windows DirectX Video Acceleration 2 路径。': 'Legacy Windows DirectX Video Acceleration 2 path.',
  'Linux/Unix 常用 VAAPI 路径；Windows 通常不可用。': 'VAAPI path commonly used on Linux/Unix; normally unavailable on Windows.',
  'OpenCL 硬件设备路径，不能保证具体输入编码可硬解。': 'OpenCL hardware-device path; it does not guarantee hardware decoding for a specific source codec.',
  'CPU 解码线程数 (-threads)': 'CPU decoder threads (-threads)',
  '仅用于限制主输入的解码线程。通常留空让 FFmpeg/解码器自动决定；它与视频编码器线程数相互独立。': 'Limits decoder threads for the primary input. Usually leave it blank for FFmpeg/the decoder to decide; it is independent from video encoder threads.',
  '解码输出格式 (-hwaccel_output_format)': 'Decoder output format (-hwaccel_output_format)',
  '通常留空。软件格式会把帧交回系统内存；硬件格式可能实现零拷贝，但当前滤镜或编码器若不接受硬件帧会失败。': 'Usually leave this blank. Software formats return frames to system memory; hardware formats may enable zero-copy but fail when a filter or encoder cannot accept hardware frames.',
  '不设置（由 FFmpeg 自动协商）': 'Unset (let FFmpeg negotiate)',
  '8-bit 4:2:0 半平面软件格式。': '8-bit 4:2:0 semi-planar software format.',
  '8-bit 4:2:0 平面软件格式，兼容性较广。': '8-bit 4:2:0 planar software format with broad compatibility.',
  '10-bit 4:2:0 半平面格式；实际名称和支持取决于解码路径。': '10-bit 4:2:0 semi-planar format; exact naming and support depend on the decoder path.',
  'Direct3D 11 硬件帧；CPU 滤镜通常不能直接处理。': 'Direct3D 11 hardware frames; CPU filters normally cannot process them directly.',
  '硬件设备参数名': 'Hardware device option',
  '仅在需要指定显卡或初始化硬件设备时设置。三种参数的值语法不同，不能互换。': 'Set this only to select a GPU or initialize a hardware device. The three options use different value syntax and are not interchangeable.',
  '不设置设备参数': 'Do not set a device option',
  '为当前主输入选择硬件加速设备。': 'Select the hardware acceleration device for the primary input.',
  '全局初始化命名硬件设备，值通常包含类型、名称和设备表达式。': 'Globally initialize a named hardware device; the value usually includes a type, name, and device expression.',
  '设置 QSV 使用的 DirectX 适配器索引、DRM 路径或 X11 display。': 'Set the DirectX adapter index, DRM path, or X11 display used by QSV.',
  '硬件设备参数值': 'Hardware device value',
  '示例：cuda=gpu:0 或 qsv=qs:hw。请按当前 FFmpeg 的 -init_hw_device 语法填写完整表达式。': 'Examples: cuda=gpu:0 or qsv=qs:hw. Enter a complete expression using the current FFmpeg -init_hw_device syntax.',
  '示例：0、1，或平台支持的设备路径。请先确认所选参数和当前平台的语法。': 'Examples: 0, 1, or a platform-supported device path. Confirm the selected option and platform syntax first.',
  '视频流复制不会解码视频，解码设置不会进入命令。': 'Video stream copy does not decode video, so decoder settings are omitted from the command.',
  '当前不输出视频，解码设置不会进入命令。': 'Video output is disabled, so decoder settings are omitted from the command.',
  '视频复制模式不执行视频解码': 'Video copy mode does not decode video',
  '当前使用 -c:v copy，压缩视频数据直接写入输出；解码面板的参数不会进入命令。切换到重新编码后才会应用。': 'With -c:v copy, compressed video is written directly to the output. Decoder settings are omitted until video is re-encoded.',
  '已选择 -vn，解码设置不会进入命令。切换到重新编码后才会应用。': '-vn is selected, so decoder settings are omitted until video is re-enabled for encoding.',
  '配置文件': 'Configuration',
  ...
  '音频容器': 'Audio containers',
  '图片容器': 'Image containers',
  '自定义容器…': 'Custom container…',
  '自定义容器后缀': 'Custom container extension',
  '直接输入文件扩展名（如 avif、m4a、webp 等），将作为输出文件的后缀': 'Enter the file extension directly (e.g. avif, m4a, webp); used as the output file suffix',
  '视频': 'Video',
  ...
  '此编解码标准在 FFmpeg 9.0 中尚无可用编码器': 'No encoders available for this codec in FFmpeg 9.0',
  'FFmpeg 9.0 尚无 AV2 编码器，保留占位': 'No AV2 encoder is available in FFmpeg 9.0 — placeholder',
  ...
  '当前 FFmpeg 未注册此编码器': 'This encoder is not registered by the selected FFmpeg',
  '当前 FFmpeg 未汇报此编码器选项': 'This option is not reported by the selected FFmpeg encoder',
  ...
  '滤镜处理格式': 'Filter processing format',
  '处理格式策略': 'Processing format strategy',
  '画面参数与滤镜': 'Frame settings and filters',
  '设置滤镜链内部采用的像素格式与精度，不选择具体画面效果。': 'Configure the pixel format and precision used inside the filter chain, separately from the visual effects themselves.',
  '兼容自动协商': 'Compatible negotiation',
  '自动高精度（推荐）': 'Automatic high precision (recommended)',
  '自定义': 'Custom',
  '控制整条视频滤镜链的内部位深、色度采样、色彩家族和最终降位方式。自动高精度至少使用 10-bit，并尽量保持输入采样结构。': 'Controls the working bit depth, chroma subsampling, color family, and final depth reduction for the whole video filter pipeline. Automatic high precision uses at least 10-bit while preserving the source sampling structure where possible.',
  '工作位深': 'Working bit depth',
  '尽量保持输入': 'Preserve input where possible',
  '16-bit 整数': '16-bit integer',
  '32-bit 浮点 RGB': '32-bit float RGB',
  '色度采样': 'Chroma subsampling',
  '工作色彩家族': 'Working color family',
  '保留 Alpha 通道': 'Preserve alpha channel',
  '最终降位抖动': 'Final depth-reduction dithering',
  '在显式输出像素格式或 CPU HDR 色调映射发生降位时生效；输出像素格式为“自动”时，最终转换仍由编码器协商。': 'Applies when an explicit output pixel format or CPU HDR tone mapping reduces bit depth. When the output pixel format is Automatic, the encoder still negotiates the final conversion.',
  '自动（误差扩散）': 'Automatic (error diffusion)',
  '不抖动': 'No dithering',
  '有序抖动': 'Ordered dithering',
  '随机抖动': 'Random dithering',
  '误差扩散': 'Error diffusion',
  '不兼容滤镜处理': 'Incompatible-filter policy',
  '阻止执行并要求调整': 'Block execution and require changes',
  '允许降级并警告': 'Allow downgrade with warning',
  ...
  '请填写完整输入时长；Desktop 可从媒体信息探测结果自动填入，网页不会读取或上传媒体文件。': 'Enter the full input duration; Desktop can fill it from Media Probe, while the website does not read or upload media files.',
  ...
  '自定义视频滤镜': 'Custom video filters',
  '自定义音频滤镜': 'Custom audio filters',
  '视频滤镜顺序': 'Video filter order',
  '音频滤镜顺序': 'Audio filter order',
  '每行填写一个完整滤镜表达式；从上到下即执行顺序，系统只负责合并为唯一 -vf，不修改或校验表达式。': 'Enter one complete filter expression per line. Top-to-bottom is execution order; the app only combines them into one -vf and does not modify or validate the expressions.',
  '每行填写一个完整滤镜表达式；从上到下即执行顺序，系统只负责合并为唯一 -af，不修改或校验表达式。': 'Enter one complete filter expression per line. Top-to-bottom is execution order; the app only combines them into one -af and does not modify or validate the expressions.',
  '一行一个表达式。可直接移动整行调整顺序；不要填写 -vf 参数名或外层引号。': 'Use one expression per line and move whole lines to reorder them. Do not include the -vf option name or outer quotes.',
  '一行一个表达式。可直接移动整行调整顺序；不要填写 -af 参数名或外层引号。': 'Use one expression per line and move whole lines to reorder them. Do not include the -af option name or outer quotes.',
  ...
  'libsvtav1 (AV1/SVT-AV1)': 'libsvtav1 (AV1/SVT-AV1)',
  'libvvenc (H.266/VVC)': 'libvvenc (H.266/VVC)',
  'mpeg2video (MPEG-2 Video)': 'mpeg2video (MPEG-2 Video)',
  'mpeg4 (MPEG-4 Part 2)': 'mpeg4 (MPEG-4 Part 2)',
  '2-Pass VBR (双遍可变码率)': '2-Pass VBR (two-pass VBR)',
  '2-Pass VBR (双遍)': '2-Pass VBR (two-pass)',
  ...
  '上传材料专用': 'Upload-ready material',
  'MP4 + H.264 Main/yuv420p + AAC 128k，目标 1900 MiB；应用后请填写素材实际时长': 'MP4 + H.264 Main/yuv420p + AAC 128k, targeting 1900 MiB; enter the actual media duration after applying',
  ...
  '多遍分析 (-multipass)': 'Multipass analysis (-multipass)',
  'disabled — 单遍': 'disabled — single pass',
  'qres — 首遍四分之一分辨率': 'qres — quarter-resolution first pass',
  'fullres — 首遍完整分辨率': 'fullres — full-resolution first pass',
