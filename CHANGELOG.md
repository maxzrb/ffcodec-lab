...
### 预设外挂 JSON 文件（Desktop）

- 内置预设与用户预设改为软件预设文件夹中的独立 JSON 文件，每个预设一个文件。
- `presets/builtin/` 存放内置预设，`presets/user/` 存放用户预设；旧版根目录遗留文件首次运行自动迁移。
- 首次打开预设管理自动写入内置种子；用户可直接编辑、新增、删除 JSON 文件，删除内置文件后不会自动恢复。
- 预设管理新增预设文件夹路径提示、「刷新」与「打开预设文件夹」；用户预设卡片显示内置徽标。
- 切换便携/用户存储模式时自动迁移预设文件。

### 真·全便携

- 存储模式标记文件由 AppData 改放程序目录；便携包整体拷贝到其他电脑后仍保持便携模式。
- 旧版 AppData 标记首次运行自动迁移；程序目录存在便携配置时自动推断便携模式。

### 验证

- Web 676/676、Desktop 53/53、TypeScript、ESLint、.NET 8 原生 helper、目录审计和 Web production build 通过。
- 新增 ini-store 与 preset-store 主进程单测，覆盖模式标记迁移、预设子目录与旧版迁移。

## v1.8.0 (2026-08-06)

### FFmpeg 8/9 实际能力兼容

- Desktop 按当前 FFmpeg 实际汇报的编码器、滤镜、编码器私有选项和通用视频编码选项动态筛选能力，不再只依赖版本号。
- 三种 NVENC 统一使用 FFmpeg 8/9 公共的 `-multipass disabled|qres|fullres`，旧 `-2pass` 配置自动迁移并与传统双遍保持独立语义。
- 修复 `-bf` 等通用视频编码选项被私有帮助页误判为不可用的问题。

### 快照式逐流管理

- 视频和音频工作台作为全局模板，可把完整编码、质量、色彩、滤镜和自定义参数快照应用到指定流。
- 快照应用后独立于全局模板，支持重新应用、载入工作台、复制到其他流和恢复继承。
- 命令生成、能力检查、诊断和编码总览统一消费逐流有效方案，并按流生成编码器和滤镜参数。

### 硬件帧与真实媒体验证

- `-hwaccel_output_format` 新增 `cuda`；D3D11/CUDA 硬件帧按媒体探针先下载为 `nv12`、`p010le` 或 `p012le`，再进入 CPU 工作格式。
- 缺少匹配探针、硬件帧直连受控 CPU 滤镜或 GPU 帧强制软件像素格式时提前阻止执行。
- 新增可重复的真实媒体矩阵，覆盖单流、多流、快照、字幕、滤镜、传统/目标大小双遍和 NVENC/AMF/QSV 探针；硬件不可用场景按能力预检记为 SKIP。

### 字幕、色彩与界面修复

- 修复 Windows 外挂字幕路径转义、ASS 不支持 `force_style`、PGS 位图字幕转文本字幕、preserve-all 字幕映射和 copy 兜底顺序。
- ffprobe 摘要补充源色彩标签；缺失必要源标签时阻止必败的 zscale 色彩转换。
- 精简快照管理卡片，删除固定不变的编辑范围提示，并统一命令预览与自由编辑的字号和等宽字体栈。

### 验证

- Web 668/668、Desktop 38/38、TypeScript、ESLint、.NET 8 原生 helper、目录审计和 Web production build 通过。
- RTX 3060 的三构建真实媒体矩阵为 PASS 100/115、SKIP 15、FAIL 0；H.264/HEVC NVENC × 三种 multipass × 三构建为 18/18 通过。
- RX 6600 双构建复核为 PASS 43/79、SKIP 36、FAIL 0；H.264/HEVC AMF 实际编码通过。

## v1.5.0 (2026-07-30)

### 滤镜精度与输入探测

- 新配置默认使用自动高精度滤镜处理，支持兼容协商、自动高精度和完整自定义三种模式。
- 支持 10/12/16-bit 与浮点、4:2:0/4:2:2/4:4:4、YUV/RGB、灰度、Alpha、降位抖动及不兼容策略。
- Desktop 媒体探测结果与滤镜格式协商联动，按输入实际像素格式缩短候选链；缺少探测时给出强提示。
- 高精度画面调整改用 `lutyuv`；已知会降为 8-bit 的 `nlmeans`、`gradfun` 默认阻止并提供明确诊断。
- 增加 D3D11 硬件帧下载边界，以及 VAAPI/Vulkan/D3D12 缺少上传上下文时的安全阻止。

### 滤镜界面与自定义链

- “滤镜处理格式”和“画面参数与滤镜”拆分为两个独立折叠卡片。
- 自定义处理设置在切换兼容/自动模式后仍完整保留。
- 自定义参数新增视频与音频滤镜卡片，每行一条表达式，分别合并为唯一 `-vf` / `-filter:a`。
- 修复自定义滤镜按 Enter 后末尾换行被受控输入回写吞掉的问题。

### 验证

- Web 630/630、Desktop 34/34 测试通过，TypeScript、ESLint、目录审计和双端 production build 通过。
- PATH FFmpeg 8.1.2 与随附构建均通过高精度像素格式、字幕、画面调整、抖动和已知降级实证。
- Full、Base、Onedir 三种 Windows 包通过版本、资源边界和压缩包完整性校验。

### 编码器高级参数

- 视频编码页新增随当前编码器变化的「编码器高级参数」折叠区，统一承载 13 个视频编码器各自的私有选项与参数说明。
- 私有参数改为显式启用：空值不进入命令，可选开关支持“不设置 / 开启 / 关闭”，避免自动生成冗长默认参数。
- 保留软件编码器原始附加参数文本框；新增通用字典合并机制，并为 SVT-AV1 Film Grain Synthesis 提供结构化强度与去噪选项。同名自由文本键由结构化值覆盖，最终只生成一个 `-svtav1-params`。
- 附加参数文本框移回视频编码栏底部；共享同一字典的结构化选项与文本内容保持双向同步。
- 视频/音频复制或禁用时新增状态说明；质量、色彩与滤镜面板不再出现没有解释的空白状态。
- 修正亮色主题中参数说明命令示例的背景和文字对比度。
...
- ESLint、TypeScript、372 项 Vitest、目录审计 0/0、10/10 代表性命令验收、FFmpeg 8.1.1 实机烟测 5/5 和生产构建全部通过。

### 命令工作台

- 命令预览新增「清空全部」按钮。确认后同步重置参数工作台，并清空自动命令与自由编辑内容；修改任意参数后自动恢复生成，避免界面与命令状态错位。
